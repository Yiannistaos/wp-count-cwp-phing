...
jQueryWeb357(document).ready(function ($) {
    ...
    counter = function (id, content) {
        ...
        if (editor_content.length == 0) {
            ...
            $('#' + id)
                .next()
                .closest('div')
                .find('.w357-charCountNoSpace')
                .text(0);
            $('#' + id)
                .next()
                .closest('div')
                .find('.w357-paragraphsCount')
                .text(0);
            $('#' + id)
                .next()
                .closest('div')
                .find('.w357-spacesCount')
                .text(0);
            return;
        }
        ...
        var spacesCount = countSpaces(editor_content);
        ...
        $('#' + id)
            ...
            .closest('div')
            .find('.w357-charCountNoSpace')
            .text(charCountNoSpace);
        $('#' + id)
            .next()
            .closest('div')
            .find('.w357-paragraphsCount')
            .text(paragraphsCount);
        $('#' + id)
            .next()
            .closest('div')
            .find('.w357-spacesCount')
    ...
    var showSpaces = parseInt(w357CountCWP.show_spaces);
    ...
    for (var i = 0; i < wp_textareas.length; i++) {
        ...
        if (showWords === 1) {
            el.innerHTML +=
                '<div class="w357-word-count w357-ml25" >' +
                w357CountCWP.words +
                ': <span class="w357-wordCount" id="wordCount-' +
                id +
                '">0</span></div>';
        }
        ...
        if (showCharacters === 1) {
            el.innerHTML +=
                '<div class="w357-char-count w357-ml25">' +
                w357CountCWP.characters +
                ': <span class="w357-charCountNoSpace" title="the count of characters excludes white spaces" id="charCountNoSpace-' +
                id +
                '">0</span></div>';
        }
        ...
        if (showParagraphs === 1) {
            el.innerHTML +=
                '<div class="w357-paragraphs-count w357-ml25">' +
                w357CountCWP.paragraphs +
                ': <span class="w357-paragraphsCount" id="paragraphsCount-' +
                id +
                '">0</span></div>';
        }
        ...
        if (showSpaces === 1) {
            el.innerHTML +=
                '<div class="w357-spaces-count w357-ml25">' +
                w357CountCWP.spaces +
                ': <span class="w357-spacesCount" id="spacesCount-' +
                id +
                '">0</span></div>';
        }
        ...
        if (showCopyrightLink === 1) {
            el.innerHTML +=
                '<div class="w357-copyright-link"><a title="Count the Characters/Words/Paragraphs while editing - A plugin developed with 🤍 by Web357" href="https://www.web357.com/product/count-words-characters-paragraphs-wordpress-plugin?utm_source=PostPage&utm_medium=MoreLink&utm_content=countcwpwp&utm_campaign=more" target="_blank">Developed by Web357</a></div>';
        }
        ...
        $('#' + id).on(
            'setcontent beforeaddundo load propertychange keyup keydown keypress blur focus input paste',
            function () {
                counter(this.id);
            }
        );
