 ...
 *****************************/
var css_admin_files = [
    'count-cwp/admin/css/admin.css',
];
var js_admin_files = [
    'count-cwp/admin/js/admin.js',
    'count-cwp/admin/js/tinymce-keyup.js',
];
var css_public_files = [
    'count-cwp/public/css/public.css'
];
var js_public_files = [
    'count-cwp/public/js/public.js'
];

// Gulp task to minify CSS files
gulp.task('minify_admin_css', function () {
    return gulp.src(css_admin_files) // or other selection
        .pipe(uglifycss())
        .pipe(rename({
            suffix: '.min'
...
gulp.task('minify_public_css', function () {
    return gulp.src(css_public_files) // or other selection
        .pipe(uglifycss())
        .pipe(rename({
            suffix: '.min'
...
gulp.task('removeprofeature', function (done) {
    ...
    setTimeout(function () {
        ...
        for (var i = 0; i < json.mirrors.length; i++) {
            ...
            gulp.src([json.mirrors[i].dst + '/**/*.{php,xml,ini,txt,css,js,scss}'])

                // GENERATOR
                .pipe(replace(/WP_PLUGIN_NAME_CLEAN/g, "Count CWP"), options)
                .pipe(replace(/WP_PLUGIN_NAME/g, "Count CWP (Pro Gulp)"), options)
                .pipe(replace(/WP_PLUGIN_URL/g, "###"), options)
                .pipe(replace(/WP_PLUGIN_SHORT_DESCRIPTION/g, "Short description for Count CWP goes here"), options)
                .pipe(replace(/WP_PLUGIN_LONG_DESCRIPTION/g, "Long description for Count CWP goes here"), options)
                .pipe(replace(/WP_PLUGIN_VERSION/g, "1.0.0"), options)
                .pipe(replace(/WP_PLUGIN_TEXT_DOMAIN_UNDERSCORE_FREE/g, "login_as_user"), options)
                .pipe(replace(/WP_PLUGIN_TEXT_DOMAIN_UNDERSCORE/g, "login_as_user_pro"), options) // login_as_user_pro or login_as_user
                .pipe(replace(/WP_PLUGIN_TEXT_DOMAIN_FREE/g, "count-cwp"), options) // count-cwp
                .pipe(replace(/WP_PLUGIN_TEXT_DOMAIN/g, "count-cwp-pro"), options) // count-cwp-pro
                ...
                .pipe(replace(/WP_PLUGIN_UPPER_CAMEL_CASE_CLASS_NAME_FREE/g, "CountCWP"), options)
                .pipe(replace(/WP_PLUGIN_UPPER_CAMEL_CASE_CLASS_NAME/g, "CountCWPPro"), options) // CountCWPPro or CountCWP
                .pipe(replace(/WP_PLUGIN_UPPERCASE_DEFINE_NAME/g, "CountCWPPRO"), options) // CountCWPPRO or COUNTCWP
                .pipe(replace(/WP_PLUGIN_DOCUMENTATION_URL/g, "#"), options)
                .pipe(replace(/WP_PLUGIN_SUPPORT_FORUM_URL/g, "#"), options)
                .pipe(replace(/WP_PLUGIN_RATE_WORDPRESS_ORG_URL/g, "#"), options)
                .pipe(replace(/WP_PLUGIN_OTHER_WEB357_WP_PLUGINS_URL/g, "#"), options)

                // README.txt
                .pipe(replace(/WP_PLUGIN_CONTRIBUTORS/g, "yiannistaos"), options)
                .pipe(replace(/WP_PLUGIN_TAGS/g, "loginasuser, web357, login, client, user"), options)
                ...
                .pipe(replace(/WP_PLUGIN_INSTALLATION_INSTRUCTIONS/g, "Installation instructions goes here..."), options)
                .pipe(replace(/WP_PLUGIN_FAQ/g, "Frequently asked questions goes here..."), options)
                .pipe(replace(/WP_PLUGIN_SCREENSHOTS/g, "Screenshots here"), options)
                .pipe(replace(/WP_PLUGIN_CHANGELOG/g, "Changelog here"), options)
                .pipe(replace(/WP_PLUGIN_CHANGELOG_URL/g, "#"), options)

                // Panta douleueis tin PRO ekdosi
                .pipe(replace(/\/\*begin:free\*\/[\s\S]*?\/*end:free\*\//gs, ""), options) // Einai PRO
